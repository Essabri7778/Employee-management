...
class Note
{
    ...
    static public function findNotes($data){
        $search = $data['search'];
        $search = '%'.$search.'%';
        try{
            $query = 'SELECT n.id, n.id_evaluation, n.id_etudiant,e.id_module, etd.nom AS etd_nom, etd.prenom, m.nom, e.type, n.valeur  FROM notes n INNER JOIN evaluation e INNER JOIN modules m INNER JOIN etudiant etd ON n.id_evaluation = e.id AND n.id_etudiant = etd.id AND e.id_module = m.id WHERE LOWER(etd.nom) LIKE :keyword OR LOWER(etd.prenom) LIKE :keyword OR LOWER(e.type) LIKE :keyword OR LOWER(m.nom) LIKE :keyword';
            $stmt = DB::connect()->prepare($query);
    ...
    {
        $query = 'SELECT e.type , m.nom, n.valeur FROM notes n INNER JOIN evaluation e ON n.id_evaluation = e.id INNER JOIN modules m ON e.id_module = m.id WHERE n.id_etudiant = ?';
        $stmt = DB::connect()->prepare($query);
        $stmt->execute([$id]);
        $notes = $stmt->fetchALl(PDO::FETCH_ASSOC);
        return json_encode($notes);
    ...
    static public function findNotesOfEtudiant($data){
        ...
        $search = '%'.$search.'%';
        try{
            $query = 'SELECT e.type , m.nom, n.valeur FROM notes n INNER JOIN evaluation e ON n.id_evaluation = e.id INNER JOIN modules m ON e.id_module = m.id WHERE n.id_etudiant = :id AND (LOWER(e.type) LIKE :keyword OR LOWER(m.nom) LIKE :keyword)';
            $stmt = DB::connect()->prepare($query);
